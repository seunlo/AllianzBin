...
class Customers extends Db
{
  ...
  {
    $sql = "UPDATE customers SET cust_full_name = ?, cust_phone_number = ?, cust_home_address = ?, cust_location = ? WHERE cust_id = ?";
    $stmt = $this->connect()->prepare($sql);
    $stmt->bindParam(1, $cust_full_name, PDO::PARAM_STR);
    ...
    $stmt->bindParam(5, $cust_id, PDO::PARAM_INT);
...
//echo "<pre>";
//print_r($customer->retrieveCustomer(40));
